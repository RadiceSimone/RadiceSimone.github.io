function check(x1, y1)        // this function returns the value of the tile using the (x1,y1) coordinates. (0,0) is in the upper left corner.
    ...
    function init()        // initialize the board
    {
      ...
      for (i = 0; i < rows * columns; i++) // Create the tiles.
      {
        ...
        tile[i].src = "img/x.png";                        // Initial picture: uncovered tile.
    ...
    function click(event) {
      ...
      {
        switch (picture(id)) {
          case 'x': tile[id].src = 'img/f.png'; remaining--; break;         // If the tile is uncovered, set a flag.
          case 'f': tile[id].src = 'img/q.png'; remaining++; break;         // If it's a flag, set a question mark.
          case 'q': tile[id].src = 'img/x.png'; break;                        // If it's a question mark, set it to uncovered.
        }
      ...
      if (event.which == 1 && picture(id) != 'f')        // On left click if the tile is not a flag:
      {
        if (board[id] == 'mine')        // if the tile is a mine:
        {
          for (i = 0; i < rows * columns; i++) {
            if (board[i] == 'mine') tile[i].src = "img/m.png";        // show all the mines,
            if (board[i] != 'mine' && picture(i) == 'f') tile[i].src = "img/e.png";        // show a strike-through mine where flags were placed incorrectly.
          }
    ...
    function reveal(index)        // Uncover the tile
    {
      ...
      tile[index].src = "img/"+ board[index] + ".png";        // Uncover the tile.
